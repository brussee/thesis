...
% Why no iOS?
The largest potential user base in the smart-phone market can be reached by targeting Android OS \cite{https://www.statista.com/statistics/266136/global-market-share-held-by-smartphone-operating-systems/}.
Android supports multiple hardware platforms, such as ARM, x86, and x86-64.
Almost all smart-phones running Android are compatible with the ARMv7 embedded application binary interface (EABI) \cite{}.
...
Our implementation uses Java, because it is the standard programming language on Android.
To run an application in the background, even when the screen is turned off, it must be run as a service.
Using Java, design requirement \ref{rq2} means we can use the Java native interface (JNI) to communicate with the C/C++ libraries and start the Python interpreter.
...
Lastly, the complete tool-chain is presented in Section \ref{sec:build_toolchain}, which is put together to build the final Android application package (APK).
...
Tribler is written entirely in Python, but Android OS does not support this natively.
As said in the design chapter we incorporate a Python interpreter into our design.
...
I changed this behavior to avoid having the GUI and back-end running in the same Python process, hindered by the GIL. %TODO <- klopt niet! multiproces kan ook
