...
% Metrics
In this experiment we measure the time needed to create a torrent file for different sizes of videos.
% Expected / desired results
...
% Setup
The setup for this measurement is exactly the same as in Section \ref{sec:content_discovery}, but we only look at the source, creating the torrent file of the video content.
% Results
...
\end{figure}
% Conclusions
The results show a linear correlation between the size of a video and the torrent creation time, with one outlier to the right.
...
Finally, to add content to a channel, only metadata is required, which does not scale with content size.
