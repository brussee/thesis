...
%	\item The implementation must be capable of running on ARMv7 or compatible mobile processors.
	...
	\item\label{rq:Connectible}  The mobile device must be connectible via WiFi or mobile data connection
	%response time
...
Since mobile platforms are not required to support running Python code natively, but the application must be distributed as a single installable package and run independently (req. \ref{rq:Independence}), the interpreter is packaged inside the application.
